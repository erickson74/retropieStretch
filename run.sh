...
if [[ "$(docker images -q retropies)" == "" ]]; then
    docker pull 18091/retropie:latest
fi
...
docker run \
  ...
  -v ~/.config/pulse:/home/retro/.config/pulse \
  -e TERM=xterm \
  $image >/dev/null
...
test=$(docker ps -l --format "{{.Names}}")

docker wait opengl-example >/dev/null
print_app_output
